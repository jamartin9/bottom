...
cfg_if! {
    if #[cfg(any(target_family = "unix", target_os = "linux"))] {
        mod process_ext;
        ...
        pub fn sysinfo_process_data(collector: &mut DataCollector) -> CollectionResult<Vec<ProcessHarvest>> {
            ...
            let _user_table = &mut collector.user_table;

            cfg_if! {
                if #[cfg(target_os = "macos")] {
                    MacOSProcessExt::sysinfo_process_data(_sys, use_current_cpu_total, unnormalized_cpu, total_memory, _user_table)
                } else if #[cfg(target_os = "freebsd")] {
                    FreeBSDProcessExt::sysinfo_process_data(_sys, use_current_cpu_total, unnormalized_cpu, total_memory, _user_table)
                } else {
